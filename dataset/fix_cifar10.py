...
def get_cifar10(
    ...
    train_labeled_idxs, train_unlabeled_idxs = train_split(
        base_dataset.targets, l_samples, u_samples, rand_number
    )
    ...
    print(
        f"#Labeled: {len(train_labeled_idxs)} #Unlabeled: {len(train_unlabeled_idxs)}"
    )
...
def train_split(labels, n_labeled_per_class, n_unlabeled_per_class, rand_number):
    ...
    for i in range(10):
        ...
        train_unlabeled_idxs.extend(
            idxs[: n_labeled_per_class[i] + n_unlabeled_per_class[i]]
        )
